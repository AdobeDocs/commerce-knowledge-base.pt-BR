...
O patch MDVA-43178 corrige o problema em que o token do cliente de um armazenamento personalizado não pode ser recuperado no GraphQL. Este patch está disponível quando a variável [Ferramenta de correções de qualidade (QPT)](/help/announcements/adobe-commerce-announcements/magento-quality-patches-released-new-tool-to-self-serve-quality-patches.md) O 1.1.14 está instalado. A ID do patch é MDVA-43178. Observe que o problema está programado para ser corrigido no Adobe Commerce 2.4.5.
...
O token do cliente não é gerado. Os comerciantes obtêm *O email do cliente fornecido não existe* em resposta.
