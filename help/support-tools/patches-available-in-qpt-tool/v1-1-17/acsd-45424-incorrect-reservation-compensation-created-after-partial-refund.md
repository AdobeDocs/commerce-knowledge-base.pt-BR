...
O patch ACSD-45424 corrige o problema em que uma compensação de reserva incorreta é criada após um reembolso parcial. Este patch está disponível quando a variável [Ferramenta de correções de qualidade (QPT)](/help/announcements/adobe-commerce-announcements/magento-quality-patches-released-new-tool-to-self-serve-quality-patches.md) O 1.1.17 está instalado. A ID do patch é ACSD-45424. Observe que o problema está programado para ser corrigido no Adobe Commerce 2.4.6.
...
1. Conclua a ordem e execute a seguinte consulta no banco de dados:
   ...
   Você obterá o registro do pedido feito no `inventory_reservation` tabela. A quantidade é 10, o que é correto.
1. Faturar esta ordem no back-end.
1. Agora crie um memorando de crédito para apenas um produto. NÃO selecione o *Retornar ao Stock* caixa de seleção
1. Execute a mesma query da Etapa 8.

<u>Resultados esperados</u>:

Se você não selecionou a variável *Retornar ao Stock* durante a criação do aviso de crédito, a variável `inventory_reservation` a tabela não terá um registro correspondente ao memorando de crédito.

<u>Resultados reais</u>:

Mesmo que você não tenha selecionado o *Retornar ao Stock* durante a criação do memorando de crédito, ele adiciona um registro ao `inventory_reservation` tabela com `creditmemo_created` tipo de evento. Além disso, o registro do memorando de crédito adicionado na variável `inventory_reservation` A tabela tem uma quantidade de 10, mesmo que você tenha criado o aviso de crédito para apenas uma quantidade.
